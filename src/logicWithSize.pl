...
belongs_to(Player, Row-Col, Board, Size) :- 
    nth0(Row, Board, RowCell),
    nth0(Col, RowCell, CellValue),
...
% horse_move(+Cell, ?NewCell, +Size)
% Determina os movimentos em L a partir de uma célula que terminem dentro dos limites do tabuleiro.
horse_move(Row-Col, NewRow-NewCol, Size) :-
    NewRow is Row-2, NewCol is Col-1, cell_in_board(NewRow-NewCol, Size).
horse_move(Row-Col, NewRow-NewCol, Size) :-
    NewRow is Row-2, NewCol is Col+1, cell_in_board(NewRow-NewCol, Size).
horse_move(Row-Col, NewRow-NewCol, Size) :-
    NewRow is Row+2, NewCol is Col-1, cell_in_board(NewRow-NewCol, Size).
horse_move(Row-Col, NewRow-NewCol, Size) :-
    NewRow is Row+2, NewCol is Col+1, cell_in_board(NewRow-NewCol, Size).
horse_move(Row-Col, NewRow-NewCol, Size) :-
    NewRow is Row-1, NewCol is Col-2, cell_in_board(NewRow-NewCol, Size).
horse_move(Row-Col, NewRow-NewCol, Size) :-
    NewRow is Row-1, NewCol is Col+2, cell_in_board(NewRow-NewCol, Size).
horse_move(Row-Col, NewRow-NewCol, Size) :-
    NewRow is Row+1, NewCol is Col-2, cell_in_board(NewRow-NewCol, Size).
...
% valid_move(+Player, +Cell, ?EndCell, +Board, +Size)
...
valid_move(_, Cell, EndCell, Board, Size) :-
    horse_move(Cell, EndCell, Size),
    empty_cell(EndCell, Board, Size).
valid_move(Player, Cell, EndCell, Board, Size) :-
    horse_move(Cell, JumpCell, Size),
    opponent_player(Player, OpponentPlayer),
    belongs_to(OpponentPlayer, JumpCell, Board, Size),
    replace(JumpCell, -1, Board, AuxBoard),
    valid_move(Player, JumpCell, EndCell, AuxBoard, Size).
...
move(Board-CurrentPlayer, Row-Col-EndRow-EndCol, NewGameState) :-
    length(Board, Size),
    belongs_to(CurrentPlayer, Row-Col, Board, Size),
    empty_cell(EndRow-EndCol, Board, Size),
    valid_move(CurrentPlayer, Row-Col, EndRow-EndCol, Board, Size),
...
valid_moves_by_cell(Board-CurrentPlayer, Moves) :-
    length(Board, Size),
    belongs_to(CurrentPlayer, PlayerCell, Board, Size),
    setof(EndCell, CurrentPlayer^PlayerCell^Board^valid_move(CurrentPlayer, PlayerCell, EndCell, Board, Size), AllUniqueEndCells),
